...
class MobileapiController extends Controller
{
    ...
    public function capture_payment(Request $request)
    {
       // $arr_data['transaction_id']    = $request->input('transaction_id');
        ...
        $booking = \DB::table('commercial_booking')->where(['id'=>$token_id])->first(); 
        ...
        {
            header("Location: http://commercial.marutiseva.com/failed.php");
            exit();
        }
        if($payment->status=="captured")
        {
            \DB::table('commercial_booking')->where(['id'=>$token_id])->update(['status'=>'Paid','booking_date'=>date('d/m/Y h:i:s A'),'transaction_id'=>$request->input('razorpay_payment_id')]);
...
';
 mail($admin, $subject1, $message1, $headers1);   //admin mail
             ...
             header("Location: http://commercial.marutiseva.com/success.php");
            exit();
        }
        else
        {
...
public function offer_capture_payment(Request $request)
    {
       // $arr_data['transaction_id']    = $request->input('transaction_id');
        ...
        $token_id = base64_decode($request->input('token_id'));
        $offer_enquiries = \DB::table('commercial_offer_enquiries')->where(['id'=>$token_id])->first();       
        ...
        if($payment->status=="captured")
        {
            \DB::table('commercial_offer_enquiries')->where(['id'=>$token_id])->update(['status'=>'Paid','booking_date'=>date('d/m/Y h:i:s A'),'transaction_id'=>$request->input('razorpay_payment_id')]);
